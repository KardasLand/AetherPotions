...
public class CustomPotionItem {
    ...
    public ItemStack build(Player player){
        ...
        itemStack = AetherPotions.instance.getNbtHandler().set(itemStack, id, "potionid");
        return itemStack;
    }

    public void givePotion(Player player, int amount){
        ItemStack potion = build(player);
        for (int i = 0; i < amount; i++){
            if (player.getInventory().firstEmpty() == -1){
                Objects.requireNonNull(player.getLocation().getWorld()).dropItemNaturally(player.getLocation(), potion);
            }else {
